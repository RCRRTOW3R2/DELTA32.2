...
const SHEET_RANGE = 'Sheet1!A:G' // Adjust range based on your sheet structure
...
export const fetchGoogleSheetsData = async (): Promise<StockData[]> => {
  try {
    ...
    // Skip header row and map to StockData
    return rows.slice(1).map((row: string[]) => ({
      symbol: row[0],
      name: row[1],
      price: parseFloat(row[2]) || 0,
      change: parseFloat(row[3]) || 0,
      changePercent: parseFloat(row[4]) || 0,
      marketCap: parseFloat(row[5]) || 0,
